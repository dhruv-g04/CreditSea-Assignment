...
export const createLoan = asyncWrapper(async (req: Request, res: Response) => {
  ...
  try {
    const newLoan = new Loan({fullName, loanAmount, loanTenure, employmentStatus, reasonForLoan, employmentAddress, status, loanOfficer });
    await newLoan.save();
...
export const getLoans = asyncWrapper(async (req: Request, res: Response) => {
  try {
...
export const updateLoanStatusVerifier = asyncWrapper(async (req: Request, res: Response) => {
  const { status, loanOfficer,_id } = req.body;
